"""Phase 3': gate re-anneal. Reset gate_net's output layer to the unsaturated
band (bias 0, small weights) so every gate is decidable again, painter frozen,
then let recon (defends busy's fine packets, worth 0.0086) vs L0 (6e-4/gate)
settle each gate PER IMAGE. From logit 0 both opening and closing are reachable
— this removes the one-way-door pathology, which is the measured blocker."""
...
# reset the decision layer to neutral
with torch.no_grad():
    m.dec.gate_net[-1].bias.fill_(0.5)          # slight open prior
    m.dec.gate_net[-1].weight.mul_(0.0).add_(0.01*torch.randn_like(m.dec.gate_net[-1].weight))
for p in m.parameters(): p.requires_grad_(False)
for p in m.dec.gate_net.parameters(): p.requires_grad_(True)
opt = torch.optim.Adam(m.dec.gate_net.parameters(), lr=3e-3)
...
t0=time.time(); snap("re-anneal start")
for ep in range(24):
    m.train()
    for x,_ in dl:
        out=m(x)
    ...
    if ep%3==2: snap(f"ep{ep+1}")
print(f"total {time.time()-t0:.0f}s", flush=True)
torch.save(m.state_dict(),"reanneal_model.pt")
json.dump(hist, open("reanneal_results.json","w"), indent=1)
